...
export default function CustomerHome() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', fontFamily:"'Plus Jakarta Sans',sans-serif", paddingBottom:72 }}>
      ...
      <div className="cd-header">
        <div className="cd-header-inner">
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
            <button className="cd-loc-btn" onClick={detectLocation}>
              ...
              <div style={{ textAlign:'left' }}>
                <div style={{ fontSize:10, fontWeight:600, color:'rgba(255,255,255,.4)', letterSpacing:'0.06em' }}>
                  {locStatus === 'detecting' ? 'DETECTING…' : 'DELIVER TO'}
                </div>
                <div style={{ fontSize:15, fontWeight:900, color:'#fff', lineHeight:1.2, maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {areaName || 'Set your area'} <span style={{ fontSize:11, fontWeight:500 }}>▾</span>
                </div>
              </div>
            </button>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <Link href="/favourites"   style={{ width:36, height:36, borderRadius:10, background:'rgba(255,255,255,.08)', display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none', fontSize:18 }}>❤️</Link>
              <Link href="/orders/history" style={{ width:36, height:36, borderRadius:10, background:'rgba(255,255,255,.08)', display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none', fontSize:18 }}>📦</Link>
              <ThemeToggle />
            </div>
          </div>
          ...
          <div style={{ position:'relative' }}>
            <span style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', fontSize:15, color:'rgba(255,255,255,.3)', pointerEvents:'none' }}>🔍</span>
            <input
            ...
            />
            {searchVal && (
              <button onClick={() => setSearchVal('')} style={{ position:'absolute', right:10, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'rgba(255,255,255,.4)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center' }}>✕</button>
            )}
          </div>
        </div>

        {/* Greeting strip */}
        <div style={{ background:'rgba(255,255,255,.03)', borderTop:'1px solid rgba(255,255,255,.06)', padding:'8px 16px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <p style={{ fontSize:12, color:'rgba(255,255,255,.4)', fontWeight:600 }}>
            {greeting}, <span style={{ color:'rgba(255,255,255,.85)', fontWeight:800 }}>{user?.name?.split(' ')[0] || 'there'}</span> 👋
          </p>
          <button onClick={async () => { await createClient().auth.signOut(); window.location.href = '/' }}
            style={{ fontSize:11, color:'rgba(255,255,255,.3)', background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
            Sign out
          </button>
        </div>
      </div>
      ...
      <div className="cd-promise">
        {PROMISES.map((p, i) => (
          <div key={i} className="cd-promise-pill">
            ...
            <span style={{ fontSize:12, fontWeight:700, color:'rgba(255,255,255,.6)' }}>{p.text}</span>
            {i < PROMISES.length - 1 && <span style={{ marginLeft:16, width:1, height:12, background:'rgba(255,255,255,.12)', display:'inline-block', flexShrink:0 }} />}
          </div>
        ))}
      </div>
        ...
        <div style={{ maxWidth:640, margin:'18px auto 0', padding:'0 16px' }}>
          <div className="cd-promo">
            <div style={{ position:'relative', zIndex:1 }}>
              ...
              <div style={{ fontSize:12, color:'rgba(255,255,255,.4)', marginTop:10 }}>Real riders. Real local shops. No dark kitchens.</div>
            </div>
      ...
      {/* ══ ORDERS ═══════════════════════════════════════════════ */}
      <div className="cd-section" style={{ borderTop:'1px solid var(--border)', paddingTop:22, marginTop:8 }}>
        <div className="cd-section-head">
          <div>
            <div className="cd-section-title">Your Orders</div>
            {orders.length > 0 && <div className="cd-section-sub">{activeOrders.length} active · {pastOrders.length} past</div>}
          </div>
          {orders.length > 3 && <Link href="/orders/history" className="cd-see-all">View all →</Link>}
        </div>

        {loading ? (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[1,2,3].map(i => <div key={i} className="cd-shimmer" style={{ height:80, borderRadius:16 }} />)}
          </div>
        ) : orders.length === 0 ? (
          <div className="cd-empty">
            <div className="cd-empty-icon">🛍️</div>
            <div className="cd-empty-title">No orders yet</div>
            <div className="cd-empty-sub">Pick a shop above and place your first order!</div>
            <button onClick={() => searchRef.current?.focus()} style={{ padding:'10px 28px', borderRadius:12, background:'#FF3008', color:'#fff', fontWeight:800, fontSize:14, border:'none', cursor:'pointer', fontFamily:'inherit' }}>Browse shops</button>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[...activeOrders, ...pastOrders].slice(0,8).map(o => <OrderCard key={o.id} order={o} active={activeOrders.includes(o)} />)}
          </div>
        )}
      </div>
  ...
  return (
    <Link href={`/stores/${product.shop_id}`} style={{ textDecoration:'none' }}>
      <div className="cd-prod-card">
        <div style={{ position:'relative' }}>
          {product.image_url
            ? <img src={product.image_url} alt={product.name} className="cd-prod-img" />
            : <div className="cd-prod-no-img"><span style={{ fontSize:38, opacity:.2 }}>🍽️</span></div>
          }
    ...
    <Link href={`/orders/${order.id}`} className={`cd-order-card${active ? ' live' : ''}`}>
      {active && (
        <div className="cd-order-live-bar">
          ...
          <span style={{ marginLeft:'auto', fontSize:11, fontWeight:700, color:'rgba(255,255,255,.9)' }}>Tap to track →</span>
        </div>
      )}
