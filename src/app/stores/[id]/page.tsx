  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: 120 }}>
      <style>{`
        ...
        .prow{transition:transform .15s,box-shadow .15s;cursor:default;}
        .prow:hover{transform:translateY(-1px);box-shadow:0 6px 24px rgba(0,0,0,.13);}
        .tab-btn{padding:12px 18px;font-weight:700;font-size:13px;background:none;border:none;cursor:pointer;font-family:inherit;white-space:nowrap;transition:color .15s;-webkit-tap-highlight-color:transparent;}
        ...
        @media(max-width:420px){
          .prod-img-wrap{width:72px!important;height:72px!important;}
          .prod-add-btn{padding:7px 14px!important;font-size:13px!important;}
        }
      `}</style>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--card-bg)', borderBottom: '1px solid var(--border)', padding: '0 20px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 12, height: 56 }}>
          <button onClick={() => router.back()}
            style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--bg-3)', border: 'none', cursor: 'pointer', fontSize: 18, color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>←</button>
      ...
      <div style={{ background: 'var(--bg-1)', borderBottom: '1px solid var(--border)' }}>
        {/* Banner */}
        {shop.banner_url && (
          <div style={{ width: '100%', height: 'clamp(140px, 20vw, 260px)', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px', display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontWeight: 900, fontSize: 'clamp(20px, 3vw, 28px)', color: 'var(--text)', marginBottom: 4, letterSpacing: '-0.02em' }}>{shop.name}</h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {[
                { v: `★ ${shop.rating}`,             col: '#d97706', bg: 'rgba(217,119,6,.14)' },
                ...
                ...(shop.min_order_amount > 0 ? [{ v: `₹${shop.min_order_amount} min order`, col: 'var(--text-3)', bg: 'var(--bg-3)' }] : []),
              ].map(b => (
                <span key={b.v} style={{ fontSize: 12, fontWeight: 700, padding: '5px 12px', borderRadius: 999, background: b.bg, color: b.col }}>{b.v}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {categories.length > 1 && (
        <div style={{ background: 'var(--card-bg)', borderBottom: '1px solid var(--border)', overflowX: 'auto', position: 'sticky', top: 56, zIndex: 40 }}>
          <div className="tab-scroll" style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', padding: '0 20px' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px' }}>
        {products.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--text-3)' }}>This shop hasn't added any products.</p>
          </div>
        )}

        {(activeCat === 'all'
          ? Object.entries(grouped)
          : [[activeCat, grouped[activeCat] ?? []]] as [string, Product[]][]
        ).map(([cat, items]) => (
          <div key={String(cat)} style={{ marginBottom: 40 }}>
            <h2 style={{ fontWeight: 900, fontSize: 16, color: 'var(--text)', marginBottom: 16, letterSpacing: '-0.01em', paddingBottom: 10, borderBottom: '1px solid var(--border)' }}>
            ...
            <div className="prod-grid" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(items as Product[]).map(product => (
                <ProductRow key={product.id} product={product}
                  qty={(cart.items as Array<{ product: { id: string }; quantity: number }>)
                    .find(i => i.product.id === product.id)?.quantity ?? 0}
        ...
        <div style={{ position: 'fixed', bottom: 20, left: 20, right: 20, zIndex: 50 }}>
          <div style={{ maxWidth: 760, margin: '0 auto' }}>
            <Link href="/cart" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#ff3008', color: '#fff', borderRadius: 18, padding: '16px 22px', textDecoration: 'none', boxShadow: '0 8px 32px rgba(255,48,8,.4)' }}>
              ...
              <span style={{ fontWeight: 900, fontSize: 16 }}>₹{cartSubtotal}</span>
            </Link>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--card-bg)', borderRadius: 20, maxWidth: 380, width: '100%', padding: '32px 28px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setDiffWarn(false)}
                style={{ flex: 1, padding: 14, borderRadius: 12, background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text-2)', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>Keep cart</button>
              <button onClick={() => { cart.clear(); if (products[0]) cart.addItem(products[0], id, shop.name); setDiffWarn(false) }}
                style={{ flex: 1, padding: 14, borderRadius: 12, background: '#ff3008', color: '#fff', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', border: 'none' }}>Clear & add</button>
            </div>
          </div>
        </div>
      )}
...
function ProductRow({ product, qty, onAdd, onRemove, onUpdate }: {
  ...
  return (
    <div className="prow" style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 16, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      {/* Image — always rendered, shows emoji if no image_url */}
      <div className="prod-img-wrap" style={{ width: 88, height: 88, borderRadius: 14, flexShrink: 0, overflow: 'hidden', background: 'var(--bg-3)', border: '1px solid var(--border)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {product.image_url ? (
          <img
            ...
            onError={e => {
              ...
              if (el.parentElement) el.parentElement.innerHTML = '<span style="font-size:32px;opacity:0.3">🍽️</span>'
            }}
          />
        ) : (
        ...
        {disc > 0 && (
          <div style={{ position: 'absolute', top: 5, left: 5, background: '#16a34a', color: '#fff', fontSize: 10, fontWeight: 800, padding: '2px 6px', borderRadius: 6 }}>-{disc}%</div>
        )}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {product.description && (
          <p style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{product.description}</p>
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 900, fontSize: 15, color: 'var(--text)' }}>₹{product.price}</span>
          {product.original_price && product.original_price > product.price && (
            <>
              <span style={{ fontSize: 13, color: 'var(--text-4)', textDecoration: 'line-through' }}>₹{product.original_price}</span>
      ...
      {/* ADD / Stepper */}
      {qty === 0 ? (
        <button onClick={onAdd} className="prod-add-btn"
          style={{ flexShrink: 0, border: '2px solid #ff3008', color: '#ff3008', background: 'none', fontWeight: 800, fontSize: 14, padding: '8px 22px', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', WebkitTapHighlightColor: 'transparent' }}>
          ADD
        </button>
      ) : (
