...
export function PhoneGate({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  async function savePhone() {
    if (!isValidPhone(phone)) { setErr('Enter a valid 10-digit Indian mobile number'); return }
    setLoading(true)
    setErr('')
    const sb = createClient()
    const { error } = await sb.from('users').update({ phone }).eq('id', userId)
    setLoading(false)
      ...
      <div style={{ background:'var(--card-bg)', borderRadius:24, padding:32, maxWidth:380, width:'100%', boxShadow:'0 24px 64px rgba(0,0,0,.2)' }}>
        ...
        <div style={{ width:56, height:56, borderRadius:18, background:'rgba(255,48,8,.1)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:20 }}>
          <svg viewBox="0 0 24 24" fill="none" width={26} height={26}>
            <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" stroke="#FF3008" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        ...
        <p style={{ fontSize:14, color:'var(--text-2)', lineHeight:1.6, marginBottom:24 }}>
          Required for delivery updates and order support.
        </p>
        ...
        <button onClick={savePhone} disabled={loading || !isValidPhone(phone)}
          style={{ width:'100%', padding:'14px', borderRadius:14, border:'none', background: isValidPhone(phone) ? '#FF3008' : 'var(--bg-3)', color: isValidPhone(phone) ? '#fff' : 'var(--text-3)', fontWeight:900, fontSize:15, cursor: isValidPhone(phone) ? 'pointer' : 'not-allowed', fontFamily:'inherit', boxShadow: isValidPhone(phone) ? '0 8px 24px rgba(255,48,8,.3)' : 'none', transition:'all .2s' }}>
